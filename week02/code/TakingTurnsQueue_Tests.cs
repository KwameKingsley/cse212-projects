...
public class TakingTurnsQueueTests
{
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: TakingTurnsQueue.GetNextPerson failed to re-enqueue people with 0 turns because 
    // it checked if (person.Turns > 1) and skipped re-enqueuing for values <= 0. PersonQueue also violated FIFO ordering.
    ...
    // Defect(s) Found: TakingTurnsQueue.GetNextPerson did not treat negative turn values (turns < 0) 
    // as infinite turns, dropping the person after 1 turn instead of re-enqueuing them. PersonQueue also violated FIFO ordering.
    ...
    // Defect(s) Found: None. The method correctly checks if the queue is empty and throws InvalidOperationException with "No one in the queue.".
